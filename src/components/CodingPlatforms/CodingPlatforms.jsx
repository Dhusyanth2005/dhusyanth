...
const CodingPlatforms = () => {
  ...
  const handleCardClick = (id) => {
    setActiveCard(id);
    // Enable this when popup is active to prevent background scrolling
  ...
  return (
    <section id="coding" className={styles.codingPlatforms}>
      ...
      {/* Separate modal that renders conditionally */}
      {activeCard !== null && (
        <div className={styles.modalOverlay}>
          <div 
            ...
            ref={modalRef}
            style={{
              '--platform-color': platforms.find(p => p.id === activeCard)?.color
            }}
          >
            <button 
            ...
            <div className={styles.platformHeader}>
              <div className={styles.headerLogo} style={{ color: platforms.find(p => p.id === activeCard)?.color }}>
                {platforms.find(p => p.id === activeCard)?.logo}
              </div>
              <h3 className={styles.headerName}>
                {platforms.find(p => p.id === activeCard)?.name}
              </h3>
            </div>
            
            <div className={styles.platformStats}>
              {platforms.find(p => p.id === activeCard)?.stats.map((stat, index) => (
